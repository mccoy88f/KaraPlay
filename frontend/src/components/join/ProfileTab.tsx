...
export function ProfileTab() {
  ...
  async function load() {
    try {
      ...
      setErr(e instanceof Error ? e.message : "Errore profilo");
    }
  }
  ...
  async function requestOtp() {
    ...
    try {
      ...
      setOtpSent(true);
      setMsg("Codice inviato: controlla la posta (se l'host ha configurato SMTP).");
    } catch (e) {
      setErr(e instanceof Error ? e.message : "Invio fallito");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function verifyOtp() {
    ...
    try {
      ...
      setMsg("Email verificata! Il tuo storico è ora collegato a questo account.");
      setOtpSent(false);
      setCode("");
      await load();
    } catch (e) {
      setErr(e instanceof Error ? e.message : "Verifica fallita");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="p-5 md:p-6">
      <h2 className="font-display text-lg font-semibold text-white">Profilo</h2>

      {stats && (
        <>
          <p className="mt-2 text-sm text-zinc-400">
            Nickname: <span className="font-medium text-white">{stats.nickname}</span>
      ...
      {stats && !stats.emailVerified && (
        <div className="mt-6 rounded-xl border border-zinc-800 bg-zinc-950/60 p-4">
          <p className="text-sm font-medium text-zinc-200">Collega la tua email</p>
          <p className="mt-1 text-xs text-zinc-500">
            Con l&apos;email verificata il punteggio resta nel tuo storico tra una serata e l&apos;altra
            (serve per il premio di fine serata).
          </p>
